...
export default {
  ...
  methods: {
    ...
    handleEnd (e) {
      const elementLeft = Math.floor(this.eleList[this.current].getBoundingClientRect().left)
      const scrollLeft = this.eleWrapper.scrollLeft
    ...
    animationScroll (distance) {
      ...
      timer = setInterval(() => {
        // 手动触发的滚动需要立即停止动画
        if (temp && this.eleWrapper.scrollLeft !== temp) {
          return clearInterval(timer)
        }
        if (distance > scrollLeft) {
          const current = this.eleWrapper.scrollLeft + step
          // 左移动
          if (current > distance || current >= maxScroll) {
            clearInterval(timer)
            this.eleWrapper.scrollLeft = distance
          } else {
            this.eleWrapper.scrollLeft = current
            temp = current
          }
        } else {
          const current = this.eleWrapper.scrollLeft - step
